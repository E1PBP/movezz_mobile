...
class ProfileRemoteDataSource {
  ...
  Future<bool> createPost({
    ...
    if (location != null && location.isNotEmpty) request.fields['location_name'] = location;
    if (hashtags != null && hashtags.isNotEmpty) request.fields['hashtags'] = hashtags;
    
    if (hours != null && hours.isNotEmpty) request.fields['time_h'] = hours;
    if (minutes != null && minutes.isNotEmpty) request.fields['time_m'] = minutes;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) print("Gagal Upload (HTTP Error ${response.statusCode}): ${response.body}");
        return false;
      }
    } catch (e) {
  ...
  Future<bool> updatePost(String postId, String newCaption) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) print("Update failed [${response.statusCode}]: ${response.body}");
        return false;
      }
    } catch (e) {
  ...
  Future<bool> deletePost(String postId) async {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (kDebugMode) print("Delete failed [${response.statusCode}]: ${response.body}");
          return false;
        }
      } catch (e) {
